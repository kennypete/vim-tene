...
|g:tene_col        .9+^.^|[.big]*0* |{h-col}[col()] (and -&#x7b;num} if %V is different)
...
|g:tene_tab_num                   |t{tab number} after buffer/window number/mode indicator
...
NOTE: `g:tene_window_num`, `g:tene_tab_num`, and `g:tene_himod` were added after the initial release of vim-tene. +
...
^|*ta*  ^|TeneA  |Prominently highlight modified [+] |_Don't_
...
^|*tt*  ^|TeneT  |t{tab number} after {h-mode}[mode] indicator |_nothing_
